...
bool validatePendingEvent(const PendingEvent& event) {
  switch (event.kind) {
    ...
    case PendingEventKind::Encounter:
      return event.recordId <= 1 && event.speciesId >= 1 && event.speciesId <= KANTO_SPECIES_COUNT &&
...
bool encounterWasPreviouslySeen(const PendingEvent& event) {
  return event.kind == PendingEventKind::Encounter && event.recordId == 1;
}
