...
bool readExact(HalFile& file, void* output, const size_t size) {
  return file.read(output, size) == static_cast<int>(size);
}

bool writeExact(HalFile& file, const void* input, const size_t size) { return file.write(input, size) == size; }
...
bool PokemonStore::writeSnapshot(const PokemonState& state, const RecordMutation& mutation, const bool discardRecords) {
  ...
  if (!writable_ || !validateState(state) || (discardRecords && mutation.kind != RecordMutationKind::None) ||
      ...
      (removing && (mutation.requestedRecordId == 0 || recordIsInParty(state, mutation.requestedRecordId))) ||
  ...
  StateBytes stateBytes{};
  const SnapshotHeader header{nextSequence, currentRecordCount + (appending ? 1U : 0U) - (removing ? 1U : 0U)};
  HeaderBytes headerBytes{};
  ...
  HalFile destination = Storage.open(destinationPath, O_WRONLY | O_CREAT | O_TRUNC);
  if (!destination) {
  ...
  bool replacementFound = !replacing && !removing;
  for (uint32_t index = 0; writeOk && index < currentRecordCount; ++index) {
    ...
    if (writeOk) {
      ...
      if (removing && recordId == mutation.requestedRecordId) {
  ...
  writeOk = writeOk && writeExact(destination, crcBytes, sizeof(crcBytes));
  if (writeOk) destination.flush();
...
uint32_t PokemonStore::nextRecordId() const {
  if (!ready_ || activeHeader_.recordCount == 0) return 1;
  HalFile file = Storage.open(activeIsA_ ? STORE_PATH_A : STORE_PATH_B, O_RDONLY);
  const size_t offset = recordsOffset(activeHeader_) +
                        (activeHeader_.recordCount - 1U) * POKEMON_RECORD_BYTES;
  RecordBytes bytes{};
  PokemonRecord record{};
  if (!file || !file.seek(offset) || !readExact(file, bytes.data(), bytes.size()) ||
      !decodeRecord(bytes, record) || record.recordId == UINT32_MAX) return 0;
  return record.recordId + 1U;
}
