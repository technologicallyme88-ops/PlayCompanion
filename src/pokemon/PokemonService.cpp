...
ServiceStatus PokemonService::movePartyMember(const uint8_t fromSlot, const uint8_t toSlot) {
  ...
  // PARTY MOVE DIRECT-SWAP FIX
  const uint32_t moved = state.partyRecordIds[fromSlot];
  state.partyRecordIds[fromSlot] = state.partyRecordIds[toSlot];
...
ServiceStatus PokemonService::releasePokemon(const uint32_t recordId) {
  PokemonState state{};
  const ServiceStatus status = loadReadyState(state);
  if (status != ServiceStatus::Ok) return status;
  if (recordId == 0) return ServiceStatus::Invalid;
  for (const uint32_t partyId : state.partyRecordIds) {
    if (partyId == recordId) return ServiceStatus::Invalid;
  }
  PokemonRecord record{};
  if (!store_.readRecord(recordId, record)) return ServiceStatus::NotFound;
  removePendingEvolutionsForRecord(state, recordId);
  const RecordMutation mutation{recordId, {}, RecordMutationKind::Remove};
  if (!store_.commit(state, mutation)) {
...
ServiceStatus PokemonService::resolveEncounter(const EncounterChoice choice, uint32_t& caughtRecordId) {
  ...
  if (choice == EncounterChoice::Catch) {
    ...
    if (mutation.requestedRecordId == 0) return ServiceStatus::StorageError;
  }
