...
PoseBounds poseInkBounds(const CompanionId id, const Mood mood) {
  ...
  int minX = SPRITE_WIDTH;
  int minY = SPRITE_HEIGHT;
  int maxX = -1;
  int maxY = -1;
  for (int row = 0; row < SPRITE_HEIGHT; ++row) {
    const uint8_t* rowBits = bits + row * SPRITE_ROW_BYTES;
    for (int col = 0; col < SPRITE_WIDTH; ++col) {
      if (((rowBits[col >> 3] >> (7 - (col & 7))) & 1) == 0) continue;
  ...
  return PoseBounds{minX, minY, maxX - minX + 1, maxY - minY + 1};
}

int poseInkWidth(const CompanionId id, const Mood mood, const int scale) {
  return poseInkBounds(id, mood).width * std::max(1, scale);
}

int poseInkHeight(const CompanionId id, const Mood mood, const int scale) {
  return poseInkBounds(id, mood).height * std::max(1, scale);
}
...
void drawPoseTrimmed(const GfxRenderer& renderer, const CompanionId id, const Mood mood, const int x, const int y,
  ...
  const PoseBounds bounds = poseInkBounds(id, mood);
  const uint8_t* bits = COMPANION_SPRITES[companionIndex][moodIndex];
  const int screenWidth = renderer.getScreenWidth();
  const int screenHeight = renderer.getScreenHeight();

  for (int localRow = 0; localRow < bounds.height; ++localRow) {
    const int srcRow = bounds.y + localRow;
    const uint8_t* rowBits = bits + srcRow * SPRITE_ROW_BYTES;
    const int baseY = y + localRow * scale;
    if (baseY + scale <= 0 || baseY >= screenHeight) continue;

    for (int localCol = 0; localCol < bounds.width; ++localCol) {
      const int sourceLocalCol = mirrored ? bounds.width - 1 - localCol : localCol;
      const int srcCol = bounds.x + sourceLocalCol;
      if (((rowBits[srcCol >> 3] >> (7 - (srcCol & 7))) & 1) == 0) continue;
      const int baseX = x + localCol * scale;
