...
// Ink-only bounds for layouts that want to use the transparent padding around
// a sprite as real screen space. This keeps integer, aspect-correct scaling.
PoseBounds poseInkBounds(CompanionId id, Mood mood);
int poseInkWidth(CompanionId id, Mood mood, int scale);
int poseInkHeight(CompanionId id, Mood mood, int scale);

// Draws only the ink-bearing bounds of a pose. The art is never stretched: both
// axes use the same integer scale, so Noodle can grow vertically without the
// wide/squashed look caused by enlarging his source art to fill the whole grid.
void drawPoseTrimmed(const GfxRenderer& renderer, CompanionId id, Mood mood, int x, int y, int scale,
                     bool mirrored = false);
